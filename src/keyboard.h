...
extern unsigned int __fastcall__ ReadKey();             // Reads a key (single press, but repeating) and returns his ASCII value
extern unsigned int __fastcall__ ReadKeyNoBounce();     // Read a single key, same as before but no repeating.
#endif
